...
print("\n" + "="*80)
print("**SIMULACION DEL DIA MIERCOLES (ESCENARIO 1 HORA)**")
print("="*80 + "\n")
...
LOG_DIR = "./logs_miercoles/"
MODEL_DIR = "./modelos_miercoles/"

os.makedirs(LOG_DIR, exist_ok=True)
os.makedirs(MODEL_DIR, exist_ok=True)

POSIBLES_CSVS = ['miercoles_hora.csv', 'Miercoles_Hora.csv', 'martes_hora.csv'] 
...
print(f"Buscando perfil objetivo para Miercoles...")
target_profile = {}
archivo_usado = None
...
for csv_file in POSIBLES_CSVS:
    if os.path.exists(csv_file):
        try:
            ...
            # Filtro Miercoles (weekday=2)
            df_dia = df_real[df_real['Timestamp'].dt.dayofweek == 2]
            
            if df_dia.empty:
                print(f"      (Sin datos de miércoles, usando todo el archivo)")
                df_dia = df_real 
            ...
            print(f"Perfil cargado de: {csv_file}")
            break 
        except Exception as e:
            print(f"      Error leyendo {csv_file}: {e}")

if not archivo_usado:
    print("Usando perfil de emergencia (20kW).")
...
class PowerComfortEnv(Env):
    ...
    def __init__(self, building_file, weather_file, target_dict, runperiod=(10, 11, 2021, 23, 11, 2022), 
...
def main():
    ...
    # =====================================================================
    print("Fase 1: Entrenamiento del agente PPO (Miercoles)...")
    ...
    checkpoint_callback = CheckpointCallback(save_freq=20000, save_path=MODEL_DIR, name_prefix="ppo_miercoles")
    
    try:
        model.learn(total_timesteps=150000, callback=checkpoint_callback)
    except: pass
    
    model.save("PPO_Agente_Miercoles_Final")
    
    # =====================================================================
    # FASE 2: GENERACION DEL DATASET DE RESULTADOS
    # =====================================================================
    print("\nGenerando CSV Miercoles...")
    try: vec_env.env_method("close")
    except: pass
    
    obs = vec_env.reset()
    current_date = datetime(2021, 11, 10, 0, 0, 0)
    end_date = datetime(2022, 11, 23, 23, 0, 0)
    data_list = []
    ...
    try:
        while current_date <= end_date:
            ...
            # Solo se registran datos correspondientes a Miercoles (weekday=2)
            if current_date.weekday() == 2:
                 data_list.append([
                     current_date.strftime("%Y-%m-%d %H:%M:%S"), 
                     t_val, p_val, actual_reward, 0
                 ])
    ...
    df.to_csv('Resultados_PPO_Miercoles_1H_Final.csv', index=False)
    print("Resultados guardados: Resultados_PPO_Miercoles_1H_Final.csv")
