...
print("\n" + "="*80)
print("**SIMULACION DEL DIA JUEVES (ESCENARIO 1 HORA)**")
print("="*80 + "\n")
...
FINAL_FILE = '/home/vboxuser/Descargas/EDIFICIO_JUEVES_V20.epJSON' 
EPW_FILE = '/home/vboxuser/Descargas/Guayaquilcl2023.epw'

LOG_DIR = "./logs_jueves/"
MODEL_DIR = "./modelos_jueves/"

os.makedirs(LOG_DIR, exist_ok=True)
os.makedirs(MODEL_DIR, exist_ok=True)

# Archivos CSV de datos reales (se busca en orden de prioridad)
POSIBLES_CSVS = ['jueves_hora.csv', 'Jueves_Hora.csv'] 
...
print(f"Buscando perfil objetivo para Jueves...")
target_profile = {}
archivo_usado = None
...
for csv_file in POSIBLES_CSVS:
    if os.path.exists(csv_file):
        try:
            ...
            # Filtro Jueves (weekday=3)
            df_dia = df_real[df_real['Timestamp'].dt.dayofweek == 3]
            
            if df_dia.empty:
                print(f"      (Sin datos de jueves, usando todo el archivo)")
                df_dia = df_real 
...
class PowerComfortEnv(Env):
    ...
    def __init__(self, building_file, weather_file, target_dict, runperiod=(4, 11, 2021, 29, 12, 2022), 
...
def main():
    ...
    # =====================================================================
    print("Fase 1: Entrenamiento del agente PPO (Jueves)...")
    ...
    checkpoint_callback = CheckpointCallback(save_freq=20000, save_path=MODEL_DIR, name_prefix="ppo_jueves")
    
    try:
        model.learn(total_timesteps=150000, callback=checkpoint_callback)
    except: pass
    
    model.save("PPO_Agente_Jueves_Final")
    
    # =====================================================================
    # FASE 2: GENERACION DEL DATASET DE RESULTADOS
    # =====================================================================
    print("\n Generando CSV Jueves...")
    try: vec_env.env_method("close")
    except: pass
    
    obs = vec_env.reset()
    current_date = datetime(2021, 11, 4, 0, 0, 0) # Jueves 4 Nov
    end_date = datetime(2022, 12, 29, 23, 0, 0)
    data_list = []
    ...
    try:
        while current_date <= end_date:
            ...
            # Solo se registran datos correspondientes a Jueves (weekday=3)
    ...
    df.to_csv('Resultados_PPO_Jueves_1H_Final.csv', index=False)
    print("Resultados guardados: Resultados_PPO_Jueves_1H_Final.csv")
