...
print("GENERANDO GRAFICAS COMPARATIVAS MARTES (15 MIN)...")

# --- 1. CARGA Y LIMPIEZA DE DATOS ---
try:
    # 1. Datos Reales
    real_file = 'Martes.csv'
    if not os.path.exists(real_file): real_file = 'martes.csv' # Fallback
    ...
    # 2. Datos Simulados (Archivo 15 min generado)
    sim_file = 'Resultados_PPO_Martes_15Min_Final.csv' 
...
stats_text = (f"Métricas Martes (15 min):\n"
              f"MAPE: {mape:.2f} %\n"
              f"RMSE: {rmse:.2f} kW\n"
              f"MAE:  {mae:.2f} kW")
...
# Hora Decimal para 15 min (ej: 10:45 -> 10.75)
...
# ==========================================
# GRÁFICA 1: PERFIL DIARIO (15 MIN)
# ==========================================
...
plt.title('Perfil Diario Promedio - Martes (Resolución 15 Min)', fontsize=16, fontweight='bold')
...
plt.tight_layout()
plt.savefig('validacion_modelo_perfil_diario_martes_15min.png', dpi=300)
...
for i, year in enumerate(years):
    ...
    if not data_year.empty:
        ...
        ax.set_title(f'Perfil Martes - Año {year} (15 min)', fontweight='bold', fontsize=14)
...
plt.xlabel('Hora del Día', fontsize=14, fontweight='bold')
plt.tight_layout()
plt.savefig('validacion_modelo_por_anio_martes_15min.png', dpi=300)
...
plt.title('Validación del Modelo - Serie Temporal Completa - Martes (15 Min)', fontsize=16, fontweight='bold')
...
plt.tight_layout()
plt.savefig('validacion_modelo_serie_temporal_martes_15min.png', dpi=300)
print("Grafica 3 lista.")
plt.close()

print("\nGraficas de Martes (15 min) generadas.")