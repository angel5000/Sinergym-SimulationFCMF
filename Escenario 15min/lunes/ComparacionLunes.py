...
print("GENERANDO GRAFICAS COMPARATIVAS LUNES (15 MIN)...")

# --- 1. CARGA Y LIMPIEZA DE DATOS ---
try:
    # 1. Datos Reales
    real_file = 'LunesVF.csv'
    ...
    # 2. Datos Simulados (El archivo FINAL generado por el Protocolo V21)
    sim_file = 'Resultados_PPO_Lunes_15Min_Final.csv' 
...
# --- 2. FUSIÓN DE DATOS ---
# Importante: Inner Join asegura que solo comparemos timestamps que existen en ambos
df_merged = pd.merge(df_real[['Act P (kW)']], df_sim[['Power_kW_PPO']], 
                     left_index=True, right_index=True, how='inner')

if df_merged.empty:
    print("Error: No hay fechas comunes. Verifica los anos de los CSV.")
    exit()

print(f"Puntos sincronizados: {len(df_merged)}")
...
# ==========================================
# GRÁFICA 1: PERFIL DIARIO PROMEDIO (15 MIN)
# ==========================================
plt.figure(figsize=(14, 8))

# Agrupar por hora decimal (0.0, 0.25, 0.5, 0.75...)
...
plt.title('Perfil Diario Promedio - Lunes (Resolución 15 Min)', fontsize=16, fontweight='bold')
...
plt.tight_layout()
plt.savefig('validacion_modelo_perfil_diario_lunes_15min.png', dpi=300)
...
for i, year in enumerate(years):
    ...
    if not data_year.empty:
        hourly_y = data_year.groupby('hour_decimal').mean().reset_index()
        xr, yr = get_smooth_curve(hourly_y['hour_decimal'], hourly_y['Act P (kW)'])
        xs, ys = get_smooth_curve(hourly_y['hour_decimal'], hourly_y['Power_kW_PPO'])
        ...
        ax.set_title(f'Perfil Lunes - Año {year} (15 min)', fontweight='bold', fontsize=14)
...
plt.xlabel('Hora del Día', fontsize=14, fontweight='bold')
plt.tight_layout()
plt.savefig('validacion_modelo_por_anio_lunes_15min.png', dpi=300)
print("Grafica 2 lista.")
plt.close()

# ==========================================
# GRÁFICA 3: SERIE TEMPORAL COMPLETA
# ==========================================
# Para la serie temporal larga, agrupamos por fecha para no saturar la gráfica
# Pero mantenemos la precisión del promedio diario
daily_points = df_merged.groupby(df_merged.index.date).mean()
dates = pd.to_datetime(daily_points.index)

plt.figure(figsize=(18, 9))

# Para serie temporal larga, usamos puntos suavizados
...
plt.title('Validación del Modelo - Serie Temporal Completa - Lunes (15 Min)', fontsize=16, fontweight='bold')
...
plt.tight_layout()
plt.savefig('validacion_modelo_serie_temporal_lunes_15min.png', dpi=300)
print("Grafica 3 lista.")
plt.close()

print("\nGraficas de Lunes (15 min) generadas.")