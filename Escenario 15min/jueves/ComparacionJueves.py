...
print("GENERANDO GRAFICAS COMPARATIVAS JUEVES (15 MIN)...")

# --- 1. CARGA Y LIMPIEZA DE DATOS ---
try:
    # 1. Datos Reales
    real_file = 'Jueves.csv'
    if not os.path.exists(real_file): real_file = 'jueves.csv'
    if not os.path.exists(real_file): real_file = 'jueves_hora.csv' # Fallback
    ...
    sim_file = 'Resultados_PPO_Jueves_15Min_Final.csv' 
...
stats_text = (f"Métricas Jueves (15 min):\n"
              f"MAPE: {mape:.2f} %\n"
              f"RMSE: {rmse:.2f} kW\n"
              f"MAE:  {mae:.2f} kW")
...
plt.title('Perfil Diario Promedio - Jueves (Resolución 15 Min)', fontsize=16, fontweight='bold')
...
plt.tight_layout()
plt.savefig('validacion_modelo_perfil_diario_jueves_15min.png', dpi=300)
...
for i, year in enumerate(years):
    ...
    if not data_year.empty:
        ...
        ax.set_title(f'Perfil Jueves - Año {year} (15 min)', fontweight='bold', fontsize=14)
...
plt.xlabel('Hora del Día', fontsize=14, fontweight='bold')
plt.tight_layout()
plt.savefig('validacion_modelo_por_anio_jueves_15min.png', dpi=300)
...
add_metrics_box(plt.gca())
plt.title('Validación del Modelo - Serie Temporal Completa - Jueves (15 Min)', fontsize=16, fontweight='bold')
...
plt.tight_layout()
plt.savefig('validacion_modelo_serie_temporal_jueves_15min.png', dpi=300)
